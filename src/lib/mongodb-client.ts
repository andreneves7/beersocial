...
import { MongoClient as MongoDriver, Db, Collection, Document, WithId } from 'mongodb';
...
class MongoDBClient {
  ...
  async connect(): Promise<void> {
    const url = process.env.MONGODB_URL || 'mongodb://beersocial:beersocial123@localhost:27017';
  ...
  private async ensureIndexes(): Promise<void> {
    if (!this.users || !this.beers || !this.reviews || !this.friendships || !this.notifications) return;
  ...
  async createBeer(beer: Omit<BeerDocument, '_id' | 'createdAt' | 'updatedAt'>): Promise<BeerDocument> {
    ...
    const doc: BeerDocument = {
      _id: `beer_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      ...beer,
      createdAt: now,
      updatedAt: now,
    };
  ...
  async addComment(reviewId: string, comment: { userId: string; userName: string; content: string }): Promise<boolean> {
    if (!this.reviews) throw new Error('MongoDB not connected');
  ...
  async getReviewsByBeer(beerId: string, limit = 20, offset = 0): Promise<ReviewDocument[]> {
    if (!this.reviews) throw new Error('MongoDB not connected');
    return this.reviews.find({ beerId }).sort({ createdAt: -1 }).skip(offset).limit(limit).toArray();
  }

  async getReviewsByUser(userId: string, limit = 20, offset = 0): Promise<ReviewDocument[]> {
    if (!this.reviews) throw new Error('MongoDB not connected');
    return this.reviews.find({ userId }).sort({ createdAt: -1 }).skip(offset).limit(limit).toArray();
  }

  async getAllReviews(limit = 20, offset = 0): Promise<ReviewDocument[]> {
    if (!this.reviews) throw new Error('MongoDB not connected');
    return this.reviews.find({}).sort({ createdAt: -1 }).skip(offset).limit(limit).toArray();
  }
}
